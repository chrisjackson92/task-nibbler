...
## 9. C# / ASP.NET Core Standards

> For backend API services using .NET and Entity Framework Core.

### 9.1 Architecture Rules

| Rule | Enforcement | Rationale |
|:-----|:------------|:----------|
| **EF entities are NEVER exposed to API controllers** | Code review, architectural linting | Prevents leaking internal schema, avoids circular serialization, keeps API contract explicit |
| **All data crossing the service boundary MUST be mapped to DTOs** | Code review | Controllers receive and return DTOs only. Services map between entities and DTOs. |
| **Controllers contain NO business logic** | Code review | Controllers delegate to services. Max responsibility: validate input, call service, return response. |
| **Services receive DTOs, return DTOs** | Code review | Services encapsulate all entity ↔ DTO mapping internally. |
| **DbContext is accessed ONLY through repositories** | Code review | Repositories encapsulate all data access. Services never reference `DbContext` directly. |
| **External API clients are ALWAYS behind interfaces** | Code review | Testability, swappability, and dependency inversion for TMDB, Open Library, etc. |

### 9.2 DTO Mapping — The Hard Rule

```csharp
// ❌ FORBIDDEN — Controller returns an EF entity
[HttpGet("{id}")]
public async Task<ActionResult<MediaItem>> GetMediaItem(Guid id)
{
    var item = await _context.MediaItems.FindAsync(id);
    return Ok(item);  // VIOLATION: EF entity leaks to API surface
}

// ✅ REQUIRED — Controller returns a DTO
[HttpGet("{id}")]
public async Task<ActionResult<MediaItemDto>> GetMediaItem(Guid id)
{
    var dto = await _mediaService.GetByIdAsync(id);
    return Ok(dto);  // Service handles entity → DTO mapping
}
```

**Why this rule exists:**
1. **EF entities have navigation properties** that cause circular serialization (e.g., `MediaItem.Books.MediaItem.Books...`)
2. **Schema changes shouldn't break the API** — the DTO is the contract, the entity is internal
3. **Selective exposure** — users should never see `password_hash`, internal IDs, or audit fields
4. **OpenAPI spec accuracy** — Swagger generates TypeScript types from DTOs, not from DB shape

### 9.3 Naming Conventions

| Element | Convention | Example |
|:--------|:-----------|:--------|
| Classes | `PascalCase` | `MediaTrackingService`, `ListController` |
| Interfaces | `IPascalCase` | `IListService`, `ITmdbClient` |
| Methods | `PascalCase` | `GetByIdAsync`, `CreateList` |
| Properties | `PascalCase` | `MediaType`, `CreatedAt` |
| Private fields | `_camelCase` | `_context`, `_logger` |
| Parameters | `camelCase` | `mediaItemId`, `userId` |
| Constants | `PascalCase` | `MaxRating`, `DefaultPageSize` |
| Async methods | `VerbAsync` suffix | `GetByIdAsync`, `CreateAsync` |
| DTOs | `EntityNameDto` / `CreateEntityRequest` / `UpdateEntityRequest` | `MediaItemDto`, `CreateListRequest` |
| Controller classes | `EntityNameController` | `ListsController`, `MediaController` |
| Service interfaces | `IEntityNameService` | `IListService`, `IStatsService` |
| Repository interfaces | `IEntityNameRepository` | `IMediaItemRepository` |

### 9.4 Async Patterns

- **All I/O operations MUST be async** — database queries, HTTP calls, file access
- **Never use `.Result` or `.Wait()`** — causes deadlocks in ASP.NET
- **Use `CancellationToken`** — propagate through all async chains

```csharp
// ✅ GOOD — Async all the way, with CancellationToken
public async Task<ListDto> GetByIdAsync(Guid id, CancellationToken ct = default)
{
    var entity = await _listRepository.GetByIdAsync(id, ct);
    if (entity is null) throw new NotFoundException($"List {id} not found");
    return _mapper.Map<ListDto>(entity);
}

// ❌ BAD — Blocking call
public ListDto GetById(Guid id)
{
    var entity = _listRepository.GetByIdAsync(id).Result; // DEADLOCK RISK
    return _mapper.Map<ListDto>(entity);
}
```
...
- **Use custom exception types** — `NotFoundException`, `ValidationException`, `ForbiddenException`
- **Global exception middleware** maps exceptions to HTTP status codes
- **Never catch-and-swallow** in services — let exceptions bubble to middleware
- **Return RFC 7807 Problem Details** for all error responses

### 9.6 Configuration

```json
// tsconfig / editorconfig equivalents for C#
{
...
- **Nullable reference types MUST be enabled** — `#nullable enable` project-wide
- **All compiler warnings treated as errors**
- **Code analysis at maximum level**

### 9.7 Tools & Enforcement

| Tool | Purpose | CI Gate |
|:-----|:--------|:--------|
| `dotnet build /warnaserror` | All warnings as errors | Zero warnings |
| .NET Analyzers (built-in) | Code quality, performance, security | Zero findings |
| StyleCop.Analyzers | Naming and formatting conventions | Zero findings |
| SonarAnalyzer.CSharp | Bugs, code smells, security hotspots | Zero findings |
| `dotnet format` | Code formatting | Pre-commit hook |

---

## 10. Error Handling Pattern (All Languages)
...
## 11. Code Review Mandate (DO-178C §6.3.4)
...
## 13. Static Analysis Rule Profiles (JPL D-60411)
...
- [ ] All public functions have docstrings/JSDoc/Doxygen/XML comments
...
- [ ] Static analysis profile enabled and passing (§13)
- [ ] Code reviewed per §11 (independent review for safety-critical)
...
- [ ] **C# only:** No EF entities exposed to controllers — DTOs only (§9.1, §9.2)
- [ ] **C# only:** DbContext accessed only through repositories (§9.1)
- [ ] **C# only:** All I/O operations are async with CancellationToken (§9.4)
- [ ] **C# only:** Nullable reference types enabled project-wide (§9.6)
- [ ] **C# only:** Custom exception types used, not generic Exception (§9.5)
