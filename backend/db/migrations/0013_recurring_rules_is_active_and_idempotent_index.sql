...
-- Unique constraint for idempotent nightly expansion (TRB-005 — AUD-008-OPS).
-- DATE(timestamptz) is STABLE (depends on session timezone) — not allowed in index expressions.
-- (COALESCE(start_at, created_at) AT TIME ZONE 'UTC')::date IS immutable ('UTC' is a constant).
CREATE UNIQUE INDEX uq_recurring_instance
  ON tasks (recurring_rule_id, (COALESCE(start_at, created_at) AT TIME ZONE 'UTC')::date)
